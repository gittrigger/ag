...
//        URI_MATCHER.addURI(getContext().getString(R.string.cp), "moment/#", SPECIFIC_MESSAGE);
//        URI_MATCHER.addURI(getContext().getString(R.string.cp), "perspective", ALL_MESSAGESPERSPECTIVE);
//        URI_MATCHER.addURI(getContext().getString(R.string.cp), "perspective/#", SPECIFIC_MESSAGEPERSPECTIVE);
//        URI_MATCHER.addURI(getContext().getString(R.string.cp), "retrospect", ALL_MESSAGESPERSPECTIVE);
//        URI_MATCHER.addURI(getContext().getString(R.string.cp), "retrospect/#", SPECIFIC_MESSAGEPERSPECTIVE);
//        URI_MATCHER.addURI(getContext().getString(R.string.cp), "filter", ALL_MESSAGESFILTER);
//        URI_MATCHER.addURI(getContext().getString(R.string.cp), "filter/#", SPECIFIC_MESSAGEFILTER);
    //"+getString(R.string.cp)+"
    }

    public static final Uri CONTENT_URI = Uri.parse( "content://com.havenskys.wave");
        	...
        	if(mDb!=null){
        		...
        		if(d==null){
        			...
        			if(d.getColumnIndex("filterid") == -1){
        			    ...
        			    mDb.execSQL("create index IF NOT EXISTS moment_filterid on moment(filterid);");
        			}

        			if(d.getColumnIndex("retrospectid") == -1){
        			    mDb.execSQL("alter table moment add column retrospectid integer");
        			    mDb.execSQL("create index IF NOT EXISTS moment_retrospectid on moment(retrospectid);");
        			}
        		...
        		mDb.execSQL("create index IF NOT EXISTS filter_hostname on filter(hostname)");
        		...
        		mDb.execSQL("create table IF NOT EXISTS retrospect (_id integer primary key autoincrement, filterid integer, filtered date, title text, url text, hostname text, docpath text, ipaddress text, port integer, responsems integer, cookies text, body text, header text, charset text, contenttype text, contentlength int, protostatus text, updated date, created date, status integer default 1 ) ");
        		mDb.execSQL("create index IF NOT EXISTS retrospect_filtered on retrospect(filtered)");
