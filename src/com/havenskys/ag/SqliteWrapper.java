...
public final class SqliteWrapper {
    ...
    public static Cursor query(Context context, ContentResolver resolver, Uri uri,
    	...
    	try {
            return resolver.query(uri, projection, selection==null?"":selection, selectionArgs, sortOrder);
    ...
    public static Uri insert(Context context, ContentResolver resolver, Uri uri, ContentValues values) {
        try {
            ...
            Log.w(TAG, "Catch a SQLiteException when insert: "+e.getLocalizedMessage());
            Log.w(TAG,"uri " + uri.toString());
            if(values.containsKey("remoteid")){Log.w(TAG,"remoteid " + values.getAsString("remoteid"));}
            checkSQLiteException(context, e);
            return null;
        }
    }
