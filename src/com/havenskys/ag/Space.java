	...
	if(footer){
		{
			...
			/*
			r1.setOnClickListener(new OnClickListener(){public void onClick(View v){Intent lookup = new Intent(mCtx,com.havenskys.ag.Lookup.class);
...
startActivity(Intent.createChooser(sendIntent, "Title:"));
	...
	Handler wayProceed = new Handler(){
		public void handleMessage(Message msg){
			...
			Intent lookup = null;
			try {
				lookup = new Intent(mCtx,Class.forName("com.havenskys.ag.Lookup"));
	...
	Handler sho = new Handler(){
		...
		public void handleMessage(Message msg){
			...
			Uri geturi = Uri.parse(getString(R.string.cp)+"/moment/"+moment);
			uri = geturi.toString();
			...
			rm.setOnClickListener(new OnClickListener(){public void onClick(View v){Intent lookup = new Intent(mCtx,com.havenskys.ag.Lookup.class);
			lookup.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_NO_HISTORY);
			lookup.putExtra("id", moment);
			if(uri != null){lookup.putExtra("uri", uri);}
			startActivity(lookup);wayGo.sendEmptyMessageDelayed(1,750);}});
